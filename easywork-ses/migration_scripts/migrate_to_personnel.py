        ...
        for employee in employees:
            try:
                async with in_transaction():
                    # 既存チェック
                    existing = await Personnel.filter(person_type=PersonType.EMPLOYEE, code=employee.code).first()
    ...
    async def migrate_freelancers(self) -> int:
        ...
        for freelancer in freelancers:
            try:
                async with in_transaction():
                    # 既存チェック
                    existing = await Personnel.filter(person_type=PersonType.FREELANCER, code=freelancer.code).first()
    ...
    async def migrate_bp_employees(self) -> int:
        ...
        for bp_employee in bp_employees:
            try:
                async with in_transaction():
                    # 既存チェック
                    existing = await Personnel.filter(person_type=PersonType.BP_EMPLOYEE, code=bp_employee.code).first()
                ...
                if evaluation.employee_id:
                    ...
                    if old_employee:
                        personnel = await Personnel.filter(
                            person_type=PersonType.EMPLOYEE, code=old_employee.code
                        ).first()
                    ...
                    if old_freelancer:
                        personnel = await Personnel.filter(
                            person_type=PersonType.FREELANCER, code=old_freelancer.code
                        ).first()

                elif evaluation.bp_employee_id:
                    old_bp = await BPEmployee.get_or_none(id=evaluation.bp_employee_id)
                    if old_bp:
                        personnel = await Personnel.filter(person_type=PersonType.BP_EMPLOYEE, code=old_bp.code).first()
