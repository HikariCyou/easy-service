...
async def get_case_history_by_case(
    ...
    """
    try:
        result = await case_history_controller.get_history_by_case(case_id=case_id, page=page, page_size=page_size)
        return Success(data=result["items"], total=result["total"])
    except Exception as e:
        return Fail(msg=str(e))
...
async def get_case_history_stats(
    ...
    try:
        search_params = CaseHistorySearchSchema(
            case_id=case_id, change_type=change_type, changed_by=changed_by, start_date=start_date, end_date=end_date
        )
...
async def export_case_history_data(
    ...
    try:
        ...
        return Success(
            data={"export_params": export_params, "download_url": None, "note": "案件変更履歴エクスポート機能は今後のバージョンで提供予定です"}
        )
    except Exception as e:
        return Fail(msg=str(e))
