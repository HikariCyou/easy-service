...
async def get_attendance_trend(
    ...
    metric: str = Query("attendance_rate", description="统计指标", pattern="^(attendance_rate|working_hours|overtime_hours|leave_days)$"),
    department: Optional[str] = Query(None, description="部门筛选"),
    person_type: Optional[str] = Query(None, description="要员类型筛选")
):
    """
...
async def get_attendance_distribution(
    analysis_type: str = Query("department", description="分析维度", pattern="^(department|person_type|age_group|nationality)$"),
    year_month: Optional[str] = Query(None, description="年月（YYYY-MM）", pattern="^\\d{4}-\\d{2}$"),
    metric: str = Query("attendance_rate", description="分析指标", pattern="^(attendance_rate|working_hours|overtime_rate|leave_rate)$")
):
    """
    考勤数据分布分析 - 多维度对比
...
async def get_attendance_ranking(
    rank_type: str = Query("attendance_rate", description="排行类型", pattern="^(attendance_rate|working_hours|overtime_hours|efficiency_score)$"),
    year_month: Optional[str] = Query(None, description="年月（YYYY-MM）", pattern="^\\d{4}-\\d{2}$"),
...
async def get_attendance_calendar_heatmap(
    ...
    metric: str = Query("attendance_count", description="热力图指标", pattern="^(attendance_count|working_hours|overtime_hours|leave_count)$"),
    ...
    """
    try:
        heatmap = await dashboard_controller.get_attendance_calendar_heatmap(
            year=year,
            metric=metric,
            user_id=user_id
        )
...
async def get_attendance_alerts(
    ...
    alert_type: Optional[str] = Query(None, description="预警类型筛选", pattern="^(attendance|overtime|leave|mood|performance)$"),
