...
class Personnel(BaseModel, TimestampMixin):
    ...
    marriage_status = fields.CharEnumField(MarriageStatus, null=True, default=MarriageStatus.SINGLE, description="婚姻状况")
    ...
    # === 稼働状况 ===
    employment_status = fields.CharEnumField(EmploymentStatus, default=EmploymentStatus.AVAILABLE, description="稼働状态")
    ...
    async def check_and_update_status_by_contracts(self):
        ...
        active_contracts = await self.contracts.filter(
            status=ContractStatus.ACTIVE, contract_start_date__lte=date.today(), contract_end_date__gte=date.today()
        ).count()
...
class EmployeeDetail(BaseModel, TimestampMixin):
    ...
    personnel = fields.OneToOneField("models.Personnel", related_name="employee_detail", description="关联要员")
...
class FreelancerDetail(BaseModel, TimestampMixin):
    ...
    personnel = fields.OneToOneField("models.Personnel", related_name="freelancer_detail", description="关联要员")
...
class BPEmployeeDetail(BaseModel, TimestampMixin):
    ...
    personnel = fields.OneToOneField("models.Personnel", related_name="bp_employee_detail", description="关联要员")
