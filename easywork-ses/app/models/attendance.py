...
class DailyAttendance(BaseModel, TimestampMixin):
    ...
    contract = fields.ForeignKeyField(
        "models.Contract", related_name="daily_attendances", description="対象契約", null=True
    )
...
class MonthlyAttendance(BaseModel, TimestampMixin):
    ...
    status = fields.CharEnumField(ApproveStatus, default=ApproveStatus.DRAFT, description="审批状态")
    ...
    async def get_daily_records(self):
        ...
        records = (
            await DailyAttendance.filter(user_id=self.user_id, work_date__gte=start_date, work_date__lte=end_date)
    ...
    def can_edit_daily_records(self) -> bool:
        """判断是否可以编辑日次记录"""
        return self.status in [ApproveStatus.DRAFT, ApproveStatus.WITHDRAWN, ApproveStatus.REJECTED]
    ...
    async def get_or_create_for_submission(cls, user_id: int, year_month: str):
        ...
        record, created = await cls.get_or_create(
            user_id=user_id, year_month=year_month, defaults={"status": ApproveStatus.DRAFT}
        )
...
class WeeklyMood(BaseModel, TimestampMixin):
    ...
    async def get_current_week_mood(cls, user_id: int):
        ...
        return await cls.get_or_none(user_id=user_id, year=year, week_number=week_number)

    @classmethod
    async def set_weekly_mood(
        cls, user_id: int, mood_status: WeeklyMoodStatus, week_number: int = None, comment: str = None
    ):
        """设置当前周的心情"""
        from datetime import datetime
        ...
        mood, created = await cls.get_or_create(
            ...
            defaults={"mood_status": mood_status, "week_number": week_number, "comment": comment},
        )
    ...
    async def get_user_mood_history(cls, user_id: int, year: int = None, limit: int = 12):
        ...
        moods = await cls.filter(user_id=user_id, year=year).order_by("-year", "-week_number").limit(limit)
    ...
    async def get_team_mood_summary(cls, user_ids: list, weeks_back: int = 4):
        ...
        moods = await cls.filter(
            user_id__in=user_ids, year=current_year, week_number__gte=start_week, week_number__lte=current_week
        ).all()
        ...
        for week_key in sorted(weekly_trends.keys()):
            ...
            trend_data.append({"week": week_key, "average_score": round(avg_score, 1), "mood_count": len(scores)})
