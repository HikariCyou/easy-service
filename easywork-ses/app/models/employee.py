...
from app.models.base import (BaseModel,
                        TimestampMixin)
from app.models.enums import EmployeeType, SalaryPaymentType, EmploymentStatus, MarriageStatus
...
class Employee(BaseModel, TimestampMixin):
    ...
    # 基本情報（他の表と統一）
    name = fields.CharField(max_length=100, null=True, description="氏名")
    ...
    employment_type = fields.CharField(
        max_length=255, null=True, default=EmployeeType.CONTRACT.value, description="雇用形態"
    )
    business_content = fields.CharField(max_length=255, null=True, description="業務内容")
    
    # 給与関連（他の表の単価に相当）
    salary_payment_type = fields.CharEnumField(
        SalaryPaymentType, default=SalaryPaymentType.MONTHLY, null=True, description="給与支給形態"
    )
    salary = fields.IntField(default=0, null=True, description="給与額")
    standard_unit_price = fields.DecimalField(
        max_digits=10, decimal_places=0, null=True, description="標準単価（月額）※プロジェクト参加時"
    )
    ...
    is_active = fields.BooleanField(default=True, description="アクティブかどうか")
    
    # HR系統固有
    process_instance_id = fields.CharField(max_length=255, null=True, description="プロセスインスタンスID")
    
    # 関連
    skills: fields.ReverseRelation["EmployeeSkill"]
    contracts: fields.ReverseRelation["Contract"]
    evaluations: fields.ReverseRelation["EmployeeEvaluation"]

    class Meta:
        table = "hr_employee"
        table_description = "自社社員管理"
        indexes = [
            ("employment_status",),
...
class EmployeeSkill(BaseModel, TimestampMixin):
    ...
    employee = fields.ForeignKeyField("models.Employee", related_name="skills", description="対象社員")
    skill = fields.ForeignKeyField("models.Skill", related_name="employees", description="技能")
    ...
    class Meta:
        table = "ses_self_employee_skill"
        table_description = "フリーランス技能関連"
        unique_together = (("employee", "skill"),)


class EmployeeEvaluation(BaseModel, TimestampMixin):
    """
    自社社員評価記録（プロジェクト終了時の評価）
    """

    freelancer = fields.ForeignKeyField("models.Employee", related_name="evaluations", description="対象社員")
    case = fields.ForeignKeyField(
        "models.Case", related_name="self_employee_evaluations", null=True, description="評価対象案件"
    )
    contract = fields.ForeignKeyField(
        "models.Contract", related_name="self_employee_evaluations", null=True, description="評価対象契約"
    )
    ...
    class Meta:
        table = "ses_self_employee_evaluation"
