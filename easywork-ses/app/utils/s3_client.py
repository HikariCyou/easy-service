    ...
    async def upload_file(self, file_path: str, s3_key: str, content_type: Optional[str] = None) -> str:
        ...
        try:
            async with self.session.client(
                ...
                # ファイルをアップロード
                await s3.upload_file(file_path, self.bucket_name, s3_key, ExtraArgs=extra_args)
    ...
    async def upload_fileobj(self, fileobj: BinaryIO, s3_key: str, content_type: Optional[str] = None) -> str:
        ...
        try:
            async with self.session.client(
                ...
                # ファイルオブジェクトをアップロード
                await s3.upload_fileobj(fileobj, self.bucket_name, s3_key, ExtraArgs=extra_args)
