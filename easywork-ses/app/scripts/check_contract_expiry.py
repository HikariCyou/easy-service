...
async def check_expired_contracts():
    ...
    expired_contracts = await Contract.filter(
        status=ContractStatus.ACTIVE, contract_end_date__lt=today
...
async def check_expiring_soon_contracts(days_ahead: int = 30):
    ...
    expiring_contracts = await Contract.filter(
        status=ContractStatus.ACTIVE, contract_end_date__gte=today, contract_end_date__lte=expiry_date
...
async def main():
    ...
    # Tortoise ORM初期化
    await Tortoise.init(
        db_url="mysql://root:password@localhost:3306/easywork_ses", modules={"models": ["app.models"]}  # 実際の設定に合わせて調整
    )
