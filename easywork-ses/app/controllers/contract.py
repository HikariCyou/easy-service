...
class ContractController:
    ...
    async def generate_contract_number(self) -> str:
        ...
        # 今日作成された契約の最大番号を取得
        latest_contract = await Contract.filter(contract_number__startswith=prefix).order_by("-contract_number").first()
    ...
    async def _handle_calculation_items(
        self, contract: Contract, calculation_items: List[ContractCalculationItemCreate]
    ) -> None:
        """
        合同精算項目の処理
        """
    ...
    async def list_contracts(self, page: int = 1, page_size: int = 10, search: Q = Q(), orders: list = []):
        query = Contract.filter(search).prefetch_related("case", "personnel")
    ...
    async def get_contract(self, id):
        contract = await Contract.get_or_none(id=id).prefetch_related("case", "personnel")
        return contract
    ...
    async def check_active_contract(self, **kwargs) -> bool:
        ...
        contract = await Contract.get_or_none(**kwargs, status=ContractStatus.ACTIVE, contract_end_date__gte=today)
