...
from app.models.bp import BPCompany, BPCompanyContract, BPSalesRepresentative
...
class BPCompanyController:
    ...
    async def get_bp_companies_with_filters(
        self, page: int = 1, page_size: int = 10, name: str = None, representative: str = None, status: str = None
    ) -> Tuple[List[Dict], int]:
        """協力会社一覧取得（フィルタ付き）"""
        query = BPCompany.all()
    ...
    async def get_bp_sales_reps_with_filters(
        ...
        total = await query.count()
        sales_reps = (
            await query.prefetch_related("bp_company")
            .order_by("-updated_at")
            .limit(page_size)
    ...
    async def get_sales_reps_by_company(self, bp_company_id: int) -> List[Dict[str, Any]]:
        """BP会社の営業担当者取得"""
        reps = await BPSalesRepresentative.filter(bp_company_id=bp_company_id, is_active=True).all()

        data = []
        for rep in reps:
            rep_dict = await rep.to_dict(
                exclude_fields=["bp_company_id", "gender", "is_active", "remark", "created_at", "updated_at"]
            )
            data.append(rep_dict)
    ...
    async def get_bp_contracts_with_filters(
        ...
        page: int = 1,
        page_size: int = 10,
        bp_company_id: int = None,
        status: str = None,
        contract_form: str = None,
    ) -> Tuple[List[Dict], int]:
        """BP会社契約一覧取得（フィルタ付き）"""
