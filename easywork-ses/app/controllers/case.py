...
class CaseController:
    ...
    async def list_cases(self, page: int = 1, page_size: int = 10, search: Q = None, order: list = []):
        ...
        cases = (
            await query.select_related("client_company", "client_sales_representative", "company_sales_representative")
            .order_by(*order)
            .limit(page_size)
    ...
    async def get_case_by_id(self, case_id: int):
        case = await Case.get_or_none(id=case_id).select_related(
            "client_company", "client_sales_representative", "company_sales_representative"
        )
        return case
                ...
                if user_id and user_id > 0:
                    ...
                    if "status" in update_dict and update_dict["status"] != old_data.get("status"):
                        await case_history_controller.create_simple_history(
                            ...
                            change_details={"old_status": old_data.get("status"), "new_status": update_dict["status"]},
                        )
...
class CaseHistoryController:
    ...
    async def get_case_history_list(
        self, page: int = 1, page_size: int = 20, search_params: CaseHistorySearchSchema = None
    ) -> Dict[str, Any]:
        """案件変更履歴一覧取得"""
        ...
        # データ取得
        histories = (
            await CaseHistory.filter(query)
            .select_related("case")
            .order_by("-created_at")
            .limit(page_size)
            .offset((page - 1) * page_size)
    ...
    async def get_history_stats(self, search_params: CaseHistorySearchSchema = None) -> Dict[str, Any]:
        ...
        for case_id, count in sorted(case_change_counts.items(), key=lambda x: x[1], reverse=True)[:10]:
            case = await Case.get_or_none(id=case_id)
            if case:
                most_changed_cases.append({"case_id": case_id, "case_title": case.title, "change_count": count})

        # Top 10ユーザー（変更回数順）
        most_active_users = []
        for user_id, count in sorted(user_change_counts.items(), key=lambda x: x[1], reverse=True)[:10]:
            most_active_users.append({"user_id": user_id, "change_count": count})

        # 最近の変更履歴（最新10件）
        recent_changes = []
        recent_histories = (
            await CaseHistory.filter(query).select_related("case").order_by("-created_at").limit(10).all()
        )
