...
from app.models.enums import ApproveStatus, AttendanceType, WeeklyMoodStatus
...
class AttendanceController:
    ...
    async def create_daily_attendance(self, attendance_data: CreateDailyAttendanceSchema) -> DailyAttendance:
        ...
        current_contract = (
            await Contract.filter(personnel=personnel, contract_end_date__gte=attendance_data.work_date)
    ...
    async def update_daily_attendance(
        self, attendance_id: int, update_data: UpdateDailyAttendanceSchema
    ) -> DailyAttendance:
        """日次出勤記録を更新"""
    ...
    async def get_daily_attendance_list(
        self, page: int = 1, page_size: int = 20, search_params: Dict = None
    ) -> Dict[str, Any]:
        """日次出勤記録一覧取得"""
        ...
        # データ取得
        attendances = (
            await DailyAttendance.filter(query)
            .prefetch_related("contract", "contract__personnel")
    ...
    async def calculate_monthly_attendance(self, contract_id: int, year_month: str) -> MonthlyAttendance:
        """月次出勤集計を計算"""
        # 既存の月次記録チェック
        monthly_attendance = await MonthlyAttendance.get_or_none(contract_id=contract_id, year_month=year_month)
    ...
    async def get_monthly_attendance_list(
        ...
        # データ取得
        monthly_attendances = (
            await MonthlyAttendance.filter(query)
            .prefetch_related("contract", "contract__personnel")
            .order_by("-year_month")
            .limit(page_size)
            .offset((page - 1) * page_size)
    ...
    async def get_attendance_calendar(self, contract_id: int, year_month: str) -> Dict[str, Any]:
        ...
        attendances = await DailyAttendance.filter(
            contract_id=contract_id, work_date__year=year, work_date__month=month
        ).all()
    ...
    async def bulk_create_attendances(
        self, attendances_data: List[CreateDailyAttendanceSchema]
    ...
    async def get_user_attendance_data(
        self, user_id: int, period_type: str = "month", target_date: Optional[date] = None
    ) -> Dict[str, Any]:
        """
        ユーザーの勤怠情報を期間別で取得
        ...
        # 該当ユーザーの現在の契約を取得
        current_contracts = (
            await Contract.filter(personnel=personnel, contract_end_date__gte=target_date)
            .prefetch_related("case", "case__client_company")
            .all()
        )
        ...
        # 勤怠データを取得
        attendance_records = (
            await DailyAttendance.filter(user_id=user_id, work_date__gte=start_date, work_date__lte=end_date)
            .prefetch_related("contract", "contract__case")
            .order_by("work_date")
            .all()
        )
    ...
    async def get_staff_list_with_attendance(
        ...
        if search_params.get("keyword"):
            ...
            query &= Q(name__icontains=keyword) | Q(name_kana__icontains=keyword) | Q(personnel_code__icontains=keyword)
        ...
        personnel_list = (
            ...
            .prefetch_related("contracts", "contracts__case", "contracts__case__client_company")
            .order_by("name")
            .limit(page_size)
            .offset((page - 1) * page_size)
            .all()
        ...
        for personnel in personnel_list:
            ...
            if personnel.person_type == "bp_employee":
                ...
                company_info = {
                    "type": "bp_company",
                    "name": personnel.bp_company.company_name
                    if hasattr(personnel, "bp_company") and personnel.bp_company
                    else "未設定",
                }
            elif personnel.person_type == "employee":
                # 自社员工
                company_info = {"type": "employee", "name": "自社"}
            elif personnel.person_type == "freelancer":
                # フリーランス
                company_info = {"type": "freelancer", "name": "フリーランス"}
            ...
            attendance_summary = await self._calculate_personnel_attendance_summary(
                personnel.user_id, target_year_month
            )
    ...
    async def _calculate_personnel_attendance_summary(self, user_id: int, year_month: str = None) -> Dict[str, Any]:
        ...
        records = await DailyAttendance.filter(
            user_id=user_id, work_date__gte=target_date, work_date__lte=last_day
        ).all()
    ...
    async def set_weekly_mood(
        self, user_id: int, mood_status: WeeklyMoodStatus, week_number: int = None, comment: str = None
    ) -> WeeklyMood:
        """设置当前周的心情状态"""
    ...
    async def get_mood_history(
        self, user_id: int, start_year: int = None, start_week: int = None, limit: int = 12
    ) -> List[WeeklyMood]:
        """获取心情历史记录（最近几周）"""
        ...
        moods = (
            await WeeklyMood.filter(user_id=user_id, year__gte=start_year - 1, year__lte=start_year)  # 跨年查询
    ...
    async def get_current_month_moods(self, user_id: int, year_month: str = None) -> Dict[str, Any]:
        ...
        if month == 1 and first_week > 50:  # 1月份可能包含去年的最后一周
            # 查询去年最后一周和今年的周
            moods_prev_year = await WeeklyMood.filter(user_id=user_id, year=year - 1, week_number=first_week).all()
            moods_current_year = await WeeklyMood.filter(user_id=user_id, year=year, week_number__lte=last_week).all()
            all_moods = list(moods_prev_year) + list(moods_current_year)
        elif month == 12 and last_week < 10:  # 12月份可能包含明年第一周
            moods_current_year = await WeeklyMood.filter(user_id=user_id, year=year, week_number__gte=first_week).all()
            moods_next_year = await WeeklyMood.filter(user_id=user_id, year=year + 1, week_number=last_week).all()
            all_moods = list(moods_current_year) + list(moods_next_year)
        else:
            # 正常情况，同一年内
            all_moods = await WeeklyMood.filter(
                user_id=user_id, year=year, week_number__gte=first_week, week_number__lte=last_week
            ).all()
    ...
    async def get_or_create_monthly_attendance(
        self, user_id: int, year_month: str, contract_id: int = None
    ) -> MonthlyAttendance:
        """获取或创建月度考勤记录"""
        # 确保year_month是字符串格式
        ...
        if not contract_id:
            ...
            if personnel:
                current_contracts = await Contract.filter(
                    personnel=personnel, contract_end_date__gte=date.today()
                ).all()
        ...
        monthly, created = await MonthlyAttendance.get_or_create(
            ...
            defaults={"contract_id": contract_id, "status": ApproveStatus.DRAFT},
        )

        return monthly

    async def _log_monthly_operation(
        self,
        monthly_id: int,
        operation: str,
        operator_id: int,
    ...
    async def submit_monthly_attendance(self, user_id: int, year_month: str, remark: str = None) -> MonthlyAttendance:
        """智能提交月次考勤（根据状态处理）"""
        monthly, created = await MonthlyAttendance.get_or_create(
            user_id=user_id, year_month=str(year_month), defaults={"status": ApproveStatus.DRAFT}
        )
    ...
    async def get_monthly_attendance_detail(self, monthly_id: int) -> Dict[str, Any]:
        ...
        daily_records = (
            await DailyAttendance.filter(user_id=monthly.user_id, work_date__gte=start_date, work_date__lte=end_date)
    ...
    async def approve_monthly_attendance(
        self, monthly_id: int, approved_by: int, remark: str = None
    ) -> MonthlyAttendance:
        """批准月度考勤（管理者操作）"""
        monthly = await MonthlyAttendance.get_or_none(id=monthly_id)
    ...
    async def reject_monthly_attendance(
        self, monthly_id: int, approved_by: int, remark: str = None
    ) -> MonthlyAttendance:
        """拒绝月度考勤（管理者操作）"""
        monthly = await MonthlyAttendance.get_or_none(id=monthly_id)
    ...
    async def get_monthly_attendance_status(self, user_id: int, year_month: str) -> Dict[str, Any]:
        ...
        # 获取或创建月度记录
        monthly, created = await MonthlyAttendance.get_or_create(
            user_id=user_id, year_month=year_month_str, defaults={"status": ApproveStatus.DRAFT}
        )
    ...
    async def get_attendance_data_for_export(
        ...
        for att in attendances:
            ...
            lunch_break = (
                f"{att.lunch_break_minutes // 60:02d}:{att.lunch_break_minutes % 60:02d}"
                if att.lunch_break_minutes > 0
                else ""
            )
            evening_break = (
                f"{att.evening_break_minutes // 60:02d}:{att.evening_break_minutes % 60:02d}"
                if att.evening_break_minutes > 0
                else ""
            )
        ...
        df = pd.DataFrame(data_rows, columns=["日付", "曜日", "開始時刻", "退勤時刻", "昼休憩", "夜休憩", "作業時間", "出勤区分", "備考"])
