...
class EmployeeEvaluationController:
    ...
    async def create_evaluation(self, evaluation_data: Dict[str, Any], evaluator_id: int) -> PersonEvaluation:
        ...
        async with in_transaction():
            # 社員存在確認
            employee = await Personnel.get_or_none(
                id=evaluation_data.get("employee_id"), person_type=PersonType.EMPLOYEE
            )
            if not employee:
    ...
    async def get_employee_evaluations(
        self, employee: Personnel, page: int = 1, page_size: int = 10
    ) -> Tuple[List[PersonEvaluation], int]:
        """特定社員の評価一覧取得"""
    ...
    async def create_employee_evaluation(
        self, employee: Personnel, evaluation_data: Dict[str, Any], evaluator_id: int
    ) -> PersonEvaluation:
        """特定社員の評価作成"""
    ...
    async def get_employee_evaluation_summary(self, employee: Personnel) -> Dict[str, Any]:
        """特定社員の評価サマリー取得"""
        evaluations = await PersonEvaluation.filter(person_type=PersonType.EMPLOYEE, person_id=employee.id).all()
        ...
        return {
            ...
            "average_independence": round(sum(independence_evals) / len(independence_evals), 2)
            if independence_evals
            else 0.0,
    ...
    async def get_top_rated_employees(self, limit: int = 10, min_evaluations: int = 3) -> List[Personnel]:
        ...
        employees = await Personnel.filter(id__in=top_employee_ids, person_type=PersonType.EMPLOYEE).all()
    ...
    async def get_evaluation_stats_by_period(self, start_date: date, end_date: date) -> Dict[str, Any]:
        ...
        evaluations = await PersonEvaluation.filter(
            person_type=PersonType.EMPLOYEE, evaluation_date__gte=start_date, evaluation_date__lte=end_date
        ).all()
