...
class PersonnelController:
    ...
    async def list_employees(
        ...
        personnel_list = await query.prefetch_related('employee_detail', 'skills__skill').order_by('-updated_at').limit(page_size).offset((page - 1) * page_size).all()
    ...
    async def get_employee_by_id(self, employee_id: int, include_details: bool = True) -> Optional[Dict[str, Any]]:
        """ID指定で社員取得"""
        personnel = await Personnel.filter(id=employee_id, person_type=PersonType.EMPLOYEE).prefetch_related('employee_detail', 'skills__skill').first()
        if personnel:
    ...
    async def list_freelancers(
        ...
        total = await query.count()
        personnel_list = await query.prefetch_related('freelancer_detail', 'skills__skill').order_by('-updated_at').limit(page_size).offset((page - 1) * page_size).all()
    ...
    async def get_freelancer_by_id(self, freelancer_id: int, include_details: bool = True) -> Optional[Dict[str, Any]]:
        """ID指定でフリーランス取得"""
        personnel = await Personnel.filter(id=freelancer_id, person_type=PersonType.FREELANCER).prefetch_related('freelancer_detail', 'skills__skill').first()
        if personnel:
            return await self._personnel_to_dict_with_details(personnel, include_details)
        return None
    ...
    async def create_freelancer(self, freelancer_data: Dict[str, Any]) -> Personnel:
        ...
        async with in_transaction():
            ...
            # 詳細情報を分離
            detail_fields = {}
            for field in ['business_name', 'tax_number', 'business_start_date', 'freelance_experience_years',
                         'preferred_project_type', 'preferred_work_style', 'ng_client_companies', 'interview_available']:
    ...
    async def get_bp_employee_by_id(self, bp_employee_id: int, include_details: bool = True) -> Optional[Dict[str, Any]]:
        """ID指定でBP社員取得"""
        personnel = await Personnel.filter(id=bp_employee_id, person_type=PersonType.BP_EMPLOYEE).prefetch_related('bp_employee_detail', 'bp_employee_detail__bp_company', 'skills__skill').first()
        if personnel:
            return await self._personnel_to_dict_with_details(personnel, include_details)
        return None
    ...
    # ===== スキル管理（全人材共通） =====
    async def get_personnel_skills(self, personnel_id: int, page: int = 1, page_size: int = 10) -> Tuple[List[PersonnelSkill], int]:
        """人材スキル一覧取得"""
        query = PersonnelSkill.filter(personnel_id=personnel_id).select_related("skill")
        
        total = await query.count()
        skills = await query.order_by("-is_primary_skill", "-proficiency", "-years_of_experience").limit(page_size).offset((page - 1) * page_size).all()
    ...
    # ===== ヘルパーメソッド =====
    async def _personnel_to_dict_with_details(self, personnel: Personnel, include_details: bool = True) -> Dict[str, Any]:
        ...
        if include_details:
            ...
            # スキル情報を追加
            personnel_skills = await PersonnelSkill.filter(personnel=personnel).select_related("skill").order_by("-is_primary_skill", "-proficiency", "-years_of_experience").all()
            skills_data = []
    ...
    async def get_available_personnel(
        self, person_type: PersonType = None, project_start_date: date = None, 
        required_skills: List[str] = None, min_experience_years: float = None,
        ...
        personnel_list = await Personnel.filter(query).order_by('-it_experience_years', '-updated_at').limit(page_size).offset((page - 1) * page_size).all()
