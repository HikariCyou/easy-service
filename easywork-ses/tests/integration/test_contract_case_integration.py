...
async def test_case_termination_with_contracts(db):
    """测试案件终了时关联契约的处理"""
    # 创建测试案件
    case = await Case.create(title="契約連動テスト案件", client_company_id=1, status=CaseStatus.OPEN)
    ...
    result = await case.terminate_case(
        termination_date=termination_date, reason="プロジェクト完了", terminated_by="プロジェクトマネージャー"
    )
...
async def test_case_termination_no_active_contracts(db):
    """测试没有有效契约时的案件终了"""
    # 创建测试案件
    case = await Case.create(title="無契約テスト案件", client_company_id=1, status=CaseStatus.OPEN)

    # 创建已终了的契约
    terminated_contract = await Contract.create(
        contract_type="BP", case=case, personnel_id=1, status=ContractStatus.TERMINATED, unit_price=500000
    )

    # 执行案件终了
    result = await case.terminate_case(termination_date=date(2024, 6, 30), reason="案件終了")
